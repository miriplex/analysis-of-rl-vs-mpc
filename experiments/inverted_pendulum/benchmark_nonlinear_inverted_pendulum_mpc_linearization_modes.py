...
from control_bench.controllers.inverted_pendulum_mpc import (
    ...
    SuccessiveLinearizedInvertedPendulumMPCController,
)
...
LINEARIZATION_MODES = ("fixed", "successive")
...
def _build_controller(mode: str, N: int):
    ...
    if mode == "fixed":
        return InvertedPendulumMPCController(
            ...
            name=f"MPC fixed N={N}",
        )
    if mode == "successive":
        return SuccessiveLinearizedInvertedPendulumMPCController(
            cfg,
            dt=nominal.dt,
            params=nonlin_params,
            u_bounds=nominal.u_bounds,
            name=f"MPC successive N={N}",
        )
    raise ValueError(f"Unknown mode: {mode}")


def _simulate_case(mode: str, N: int, x0: np.ndarray) -> dict:
    _, nonlin_params = _bounded_params()
    plant = build_nonlinear_inverted_pendulum(nonlin_params)
    controller = _build_controller(mode, N)
    exp = INVERTED_PENDULUM_NONLINEAR_MPC_EXPERIMENT
...
def _write_outputs(case_rows: list[dict], summary_rows: list[dict]) -> tuple[Path, Path]:
    out_dir = EXPERIMENTS_ROOT / "results" / "metrics" / "inverted_pendulum_mpc_successive"
    out_dir.mkdir(parents=True, exist_ok=True)
    case_csv = out_dir / "nonlinear_local_case_results.csv"
    summary_csv = out_dir / "nonlinear_local_mode_summary.csv"
    summary_md = out_dir / "nonlinear_local_mode_summary.md"
    ...
    for row in summary_rows:
        lines.append(
            f"| {row['mode']} | {row['N']} | {row['cases_passed']}/{row['cases_total']} | "
            f"{row['cases_survived']}/{row['cases_total']} | {row['pass_fraction']:.3f} | "
            f"{row['survival_fraction']:.3f} | {row['max_final_abs_x_m']:.3f} | "
...
def main() -> None:
    ...
    for mode in LINEARIZATION_MODES:
        for N in HORIZON_SWEEP:
            per_group = [_simulate_case(mode, N, x0) for x0 in cases]
            case_rows.extend(per_group)
            passed = sum(1 for row in per_group if row["pass_case"])
            survived = sum(1 for row in per_group if row["survived_horizon"])
            summary_rows.append(
                {
                    "mode": mode,
                    "N": int(N),
                    "cases_total": len(per_group),
                    "cases_passed": passed,
                    "cases_survived": survived,
                    "pass_fraction": float(passed / len(per_group)),
                    "survival_fraction": float(survived / len(per_group)),
                    "max_final_abs_x_m": float(max(abs(row["final_x_m"]) for row in per_group)),
                    "max_final_abs_theta_deg": float(max(abs(row["final_theta_deg"]) for row in per_group)),
                }
            )

    summary_rows.sort(key=lambda row: (row["mode"], row["N"]))
    summary_csv, _ = _write_outputs(case_rows, summary_rows)
    print(f"Saved nonlinear local mode benchmark to: {summary_csv}")
    for row in summary_rows:
        print(
            f"mode={row['mode']:10s} | N={row['N']:2d} | "
