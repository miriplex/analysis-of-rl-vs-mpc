...
T_FINAL = INVERTED_PENDULUM_MPC_EXPERIMENT.t_final
X0 = np.array(INVERTED_PENDULUM_MPC_EXPERIMENT.x0, dtype=float)
REFERENCE = np.array(INVERTED_PENDULUM_MPC_EXPERIMENT.reference, dtype=float)
...
ANGLE_FAILURE_LIMIT_RAD = INVERTED_PENDULUM_MPC_EXPERIMENT.angle_limit_rad
CART_FAILURE_LIMIT_M = INVERTED_PENDULUM_MPC_EXPERIMENT.cart_limit_m
...
def run() -> None:
    ...
    ax_u = fig.add_subplot(grid[2, 1], sharex=ax_state)
    ...
    state_lines = [
        ax_state.plot([], [], linewidth=2.4, color="#1d4ed8", label=labels[0])[0],
        ax_rate.plot([], [], linewidth=2.4, color="#ea580c", label=labels[1])[0],
        ax_state.plot([], [], linewidth=2.4, color="#16a34a", label=labels[2])[0],
        ax_rate.plot([], [], linewidth=2.4, color="#dc2626", label=labels[3])[0],
    ]
    u_line = ax_u.plot([], [], linewidth=2.4, color="#991b1b", label="u [N]")[0]
    ax_state.grid(True, alpha=0.3)
    ax_rate.grid(True, alpha=0.3)
    ax_u.grid(True, alpha=0.3)
    ax_state.axhline(0.0, color="0.7", linewidth=1.2, linestyle="--")
    ax_rate.axhline(0.0, color="0.7", linewidth=1.2, linestyle="--")
    ax_u.axhline(0.0, color="0.7", linewidth=1.2, linestyle="--")
    ...
    ax_u.set_title("Control force", fontsize=15, pad=8)
    ...
    fig.suptitle("Linearized Inverted Pendulum with MPC", fontsize=18, y=0.985)
    ...
    for k, tk in enumerate(t):
        obs = plant.observe()
        u = controller.step(r=REFERENCE, obs=obs, t=tk)
        ...
        state_lines[0].set_data(t[: k + 1], x_hist[: k + 1, 0])
        state_lines[1].set_data(t[: k + 1], x_hist[: k + 1, 1])
        state_lines[2].set_data(t[: k + 1], x_hist[: k + 1, 2])
        state_lines[3].set_data(t[: k + 1], x_hist[: k + 1, 3])
        u_line.set_data(t[: k + 1], u_hist[: k + 1])
        ...
        if not np.all(np.isfinite(x_hist[k])) or not np.isfinite(u_hist[k]):
            stop_reason = "Stopped: state or control became non-finite."
            break
    ...
    out_path = out_dir / "closed_loop_linearized_inverted_pendulum_mpc.png"
