...
from control_bench.config import (
    ...
    INVERTED_PENDULUM_WIND_DISTURBANCE,
)
...
T_FINAL = INVERTED_PENDULUM_WIND_DISTURBANCE.t_final
X0 = np.array(INVERTED_PENDULUM_WIND_DISTURBANCE.x0, dtype=float)
REFERENCE = np.array(INVERTED_PENDULUM_WIND_DISTURBANCE.reference, dtype=float)
...
ANGLE_FAILURE_LIMIT_RAD = INVERTED_PENDULUM_WIND_DISTURBANCE.angle_limit_rad
CART_FAILURE_LIMIT_M = INVERTED_PENDULUM_WIND_DISTURBANCE.cart_limit_m
CART_STOP_TEXT = "none" if not np.isfinite(CART_FAILURE_LIMIT_M) else f"{CART_FAILURE_LIMIT_M:.2f} m"
WIND_START_S = INVERTED_PENDULUM_WIND_DISTURBANCE.wind_start_s
WIND_DURATION_S = INVERTED_PENDULUM_WIND_DISTURBANCE.wind_duration_s
WIND_PEAK_FORCE = INVERTED_PENDULUM_WIND_DISTURBANCE.wind_peak_force
...
def _weight_path(kind: str) -> Path:
    weights_dir = EXPERIMENTS_ROOT / "results" / "rl_numpy" / "inverted_pendulum_linearized" / "per_plant"
    suffix = "pidfeat" if kind == "pidfeat" else "rich"
    return weights_dir / f"rl_{suffix}__{INVERTED_PENDULUM_RL_TRAINING.plant_id}.npz"


def _load_controller(*, kind: str, plant) -> InvertedPendulumRLController:
    weight_path = _weight_path(kind)
    _require_file(weight_path)
    if kind == "pidfeat":
...
def run_rl_wind_demo(*, kind: str, figure_name: str) -> None:
    ...
    state_text = ax_anim.text(text_x, visual_pole_length - 0.14, "", fontsize=10, family="monospace")
    ...
    rl_line, = ax_force.plot([], [], linewidth=2.4, color="#991b1b", label="u_rl [N]")
    ...
    ax_force.set_title("RL force, wind, and applied total", fontsize=15, pad=8)
    ...
    fig.suptitle(f"Linearized Inverted Pendulum with {controller.name} Under Wind Disturbance", fontsize=18, y=0.985)
    ...
    for k, tk in enumerate(t):
        obs = plant.observe()
        u_rl = controller.step(r=REFERENCE, obs=obs, t=tk)
        d_wind = _wind_force(tk)
        u_total = float(u_rl[0]) + d_wind

        x_hist[k] = plant.state
        u_rl_hist[k] = float(u_rl[0])
        d_hist[k] = d_wind
        ...
        bob_x = cart_x - visual_pole_length * np.sin(theta)
        bob_y = visual_pole_length * np.cos(theta)

        cart_patch.set_xy((cart_x - cart_width / 2.0, -cart_height / 2.0))
        pole_line.set_data([cart_x, bob_x], [0.0, bob_y])
        ...
        rl_line.set_data(t[: k + 1], u_rl_hist[: k + 1])
        ...
        if abs(float(x_hist[k, 2])) > ANGLE_FAILURE_LIMIT_RAD:
            stop_reason = (
                "Stopped: pendulum left the linear region "
