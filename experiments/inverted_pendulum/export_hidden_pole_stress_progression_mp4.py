...
HOLD_DURATION_S = max(0.0, STAGE_DURATION_S - BENCHMARK_T_FINAL)
HIDDEN_POLE = 100.0
MAX_HIDDEN_ORDER = 8
TRACK_HALF_WIDTH = 0.75
OUT_DIR = EXPERIMENTS_ROOT / "results" / "figures" / "inverted_pendulum"
OUT_PATH = OUT_DIR / "hidden_pole_stress_progression_grid.mp4"
PREVIEW_PATH = OUT_DIR / "hidden_pole_stress_progression_grid_preview.png"
...
def _pretty_failure_reason(reason: str) -> str:
    ...
    labels = {
        ...
        "nonfinite_applied_control": "non-finite applied control",
...
def _simulate_case_full(*, controller, x0: np.ndarray, case_label: str, hidden_order: int) -> CaseRun:
    ...
    plant.reset(x0=x0)
    controller.reset()
    hidden_chain = build_hidden_pole_chain(hidden_pole=HIDDEN_POLE, hidden_order=hidden_order, dt=dt)
    hidden_chain.reset()
    ...
    for k, tk in enumerate(t):
        ...
        u_cmd = controller.step(r=REFERENCE, obs=obs, t=float(tk))
        ...
        u_applied = float(hidden_chain.step(float(u_cmd[0])))
...
def _build_stage_data() -> tuple[list[StageData], list, list[str]]:
    ...
    for hidden_order in range(1, MAX_HIDDEN_ORDER + 1):
        ...
        next_survivors: list = []
        logs.append(f"n={hidden_order}: {len(survivors)} entering")
        ...
        stages.append(StageData(hidden_order=hidden_order, panels_by_variant=stage_panels))
...
def _configure_stage(
    *,
    ...
    for ax, artists, spec in zip(axes, panel_artists, slot_specs):
        ...
        is_active = spec.variant_id in active_variant_ids
...
def _set_stage_frame(
    *,
    ...
    time_text.set_text(f"hidden order n = {stage.hidden_order} | stage t = {local_time_s:4.2f} s")
...
def run() -> None:
    ...
    if not stages:
        raise RuntimeError("No hidden-pole stages were generated.")
    ...
    fig.suptitle("Hidden-pole stress progression on the nonlinear inverted pendulum", fontsize=24, y=0.985)
    fig.text(
        0.5,
        0.953,
        "10 s per hidden-order stage; red frame = benchmark fail at the current hidden order; only survivors continue",
        ha="center",
        va="top",
        fontsize=13,
        color="#4b5563",
    ...
    writer = FFMpegWriter(
        ...
        codec="libx264",
        bitrate=3200,
        metadata={"title": "Hidden-pole stress progression", "artist": "Codex"},
