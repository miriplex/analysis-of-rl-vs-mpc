...
T_FINAL = INVERTED_PENDULUM_NONLINEAR_MPC_EXPERIMENT.t_final
X0 = np.array(INVERTED_PENDULUM_NONLINEAR_MPC_EXPERIMENT.x0, dtype=float)
REFERENCE = np.array(INVERTED_PENDULUM_NONLINEAR_MPC_EXPERIMENT.reference, dtype=float)
...
ANGLE_FAILURE_LIMIT_RAD = INVERTED_PENDULUM_NONLINEAR_MPC_EXPERIMENT.angle_limit_rad
CART_FAILURE_LIMIT_M = INVERTED_PENDULUM_NONLINEAR_MPC_EXPERIMENT.cart_limit_m
...
def run() -> None:
    ...
    u_line, = ax_u.plot([], [], linewidth=2.4, color="#991b1b", label="u [N]")

    for ax in (ax_state, ax_rate, ax_u):
    ...
    fig.suptitle("Nonlinear Inverted Pendulum with Linear MPC", fontsize=18, y=0.985)
    ...
    out_path = out_dir / "closed_loop_nonlinear_inverted_pendulum_mpc.png"
