...
from control_bench.experiment_manifest import first_order_input_noise_stress_test
from disturbance_stress_common import run_stress_test, stress_scale_values


CFG = first_order_input_noise_stress_test()
T_FINAL = float(CFG["t_final_seconds"])
R_STEP = float(CFG["reference_step"])
BASE_SIGMA_DU = float(CFG["base_disturbance_noise_std"])
...
def _trace_builder(plant_id: str, n_steps: int, dt: float, stress_scale: float) -> Tuple[np.ndarray, np.ndarray]:
    del dt
    seed_pid = (int(SEED) + int(zlib.crc32(f"input::{plant_id}".encode("utf-8")))) % (2**32)
    ...
    du = float(stress_scale) * float(BASE_SIGMA_DU) * base_eps_du
    return noise_y, du
...
def main() -> None:
    ...
    run_stress_test(
        ...
        summary_lines=(
            ...
            f"- Base input disturbance-noise standard deviation: `{BASE_SIGMA_DU:g}`",
        ),
        heatmap_xlabel="Input-noise multiplier on the baseline disturbance-noise standard deviation",
        heatmap_title="Input-noise stress test | pass fraction across first-order plants",
        progress_label="input-noise stress",
        scenario_prefix="input_noise_scale",
